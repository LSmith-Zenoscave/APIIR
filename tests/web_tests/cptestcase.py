"""
Taken from Sylvian Hellegouarch's Recipe @testing/unit/serverless/cptestcase.py
...
# Not strictly mandatory but just makes sense
...
class BaseCherryPyTestCase(unittest.TestCase):
    def request(self, path='/', method='GET', app_path='', scheme='http',
        ...
        * CherryPy is multi-threaded. The response you will get
        ...
        * Responses are dispatched to a mounted application's
        ...
        if not app:
            raise AppError("No application mounted at '%s'" % app_path)
        ...
        try:
            ...
            response = request.run(method, str(path), qs, proto, h, fd)
        finally:
            if fd:
                fd.close()
                fd = None
